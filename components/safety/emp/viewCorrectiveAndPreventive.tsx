...
const ViewCorrectiveAndPreventive = () => {
    const [result, setResult] = useState([]);
    useEffect(() => {
        const fn = async () => {
          const resp = await weeklyAuditAction.FETCH.fetchCorrectiveAndPrev();
          console.warn(resp);
          if (resp.data) {
    ...
    const handleDelete = async(id:any) => {
        const resp = await weeklyAuditAction.DELETE.deleteCorrectiveAndPrev(id)
        if(resp.success){
...
export default ViewCorrectiveAndPreventive
