...
const PurchaseChemical = () => {
  ...
  useEffect(()=>{
    const fn = async() => {
        const resp = await chemicalAction.FETCH.fetchChemicalPurchases();       
        if(resp.success){
  ...
  const handleDelete = async(id:any) => {
    const resp = await chemicalAction.DELETE.deleteChemicalPurchase(id)
    if(resp.success){
  ...
  const handleSubmit = async(e) => {
    e.preventDefault();
    const resp = await chemicalAction.CREATE.createChemicalPurchase(
      JSON.stringify(
        {
...
export default PurchaseChemical
