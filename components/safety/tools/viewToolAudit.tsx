...
const ViewToolAudit = () => {
    ...
    useEffect(() => {
        const fn = async () => {
          const resp = await SafetyToolsAction.FETCH.fetchAuditList();
          console.warn(resp);
          if (resp.data) {
    ...
    const handleDelete = async(id:any) => {
        const resp = await SafetyToolsAction.DELETE.deleteAuditById(id);
        if(resp.success){
    ...
    {result && (
        <>
          <div className="flex flex-col items-center justify-center mt-6">
            <h2 className="text-3xl">List Of Audit</h2>
            {result.length === 0 && (
