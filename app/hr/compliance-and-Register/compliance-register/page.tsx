  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      const compliancesByMonthYear =
        await EmployeeDataAction.FETCH.fetchCompliances(month - 1, year);
        ...
        setData(
          allData
            ...
            .map((item, index) => ({
              ...
              basicAndDA:
                parseFloat(item.designation.basic) +
                  parseFloat(item.designation.DA) || 0,
              otherPayment: parseFloat(item.otherCash) || 0,
              incentive: item.incentiveAmount || 0,
              grossPayment: item.netAmountPaid || 0,
              pf:
                (parseFloat(item.designation.basic) +
                  parseFloat(item.designation.DA)) *
                  0.12 || 0, // Assuming 12% PF
