...
const ViewCheckList = () => {
    ...
    useEffect(() => {
        const fn = async () => {
          const resp = await SafetyToolsAction.FETCH.fetchCheckList();
          console.warn(resp);
          if (resp.data) {
    ...
    const handleDelete = async(id:any) => {
        const resp = await SafetyToolsAction.DELETE.deleteCheckListById(id);
        if(resp.success){
    ...
    {result && (
        <>
          <div className="flex flex-col items-center justify-center mt-6">
            <h2 className="text-3xl">List Of CheckList</h2>
            {result.length === 0 && (
              <span className="mt-4">No CheckList Present</span>
            )}
