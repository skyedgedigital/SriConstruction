...
const IssueTool = () => {
  ...
  useEffect(()=>{
    const fn = async() => {
        const resp = await SafetyToolsAction.FETCH.fetchSafetyToolsIssues();       
        if(resp.success){
  ...
  const handleDelete = async(id:any) => {
    const resp = await SafetyToolsAction.DELETE.deleteSafetyToolIssue(id)
    if(resp.success){
  ...
  const handleSubmit = async(e) => {
    e.preventDefault();
    const resp = await SafetyToolsAction.CREATE.createSafetyToolIssue(
      JSON.stringify(
        {
