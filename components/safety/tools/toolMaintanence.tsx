...
const ToolMaintanence = () => {
  ...
  const generateExcel = () => {
    ...
    XLSX.writeFile(workbook, `Tool_Maintanence${selectedDate || "No_Date"}.xlsx`);
    ...
    const storageRef = ref(
      storage,
      `excel/Tool_Maintanence${selectedDate || "No_Date"}.xlsx`
    );
    ...
    uploadTask.on(
      ...
      () => {
        getDownloadURL(uploadTask.snapshot.ref).then(async(downloadURL) => {
          ...
          const resp = await SafetyToolsAction.CREATE.createMaintanence(JSON.stringify(obj))
          if(resp.success){
  ...
  return (
    <>
      <div className="text-2xl flex justify-center my-4">Tool Maintanence</div>
