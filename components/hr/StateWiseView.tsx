  ...
  return (
    <>
      <div className='w-full flex flex-col'>
        <h2 className='flex justify-center text-2xl'>List of States</h2>
        {uniqueStates.map((state) => (
          ...
          >
            ...
            <span className='ml-2'>{groupedByState[state]?.length || 0} Work Orders</span>
            <button
          ...
          <div className="relative w-auto my-6 mx-auto max-w-3xl">
            <div className="rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
              ...
              <div className="flex items-start justify-between p-5 border-b border-solid border-blueGray-200 rounded-t">
                <h3 className="text-2xl font-semibold">{selectedState} - Work Orders</h3>
                <button
              ...
              <div className="relative p-6 flex flex-col max-h-[70vh] overflow-y-auto">
                {groupedByState[selectedState].map((workOrder) => (
                  <div key={workOrder._id} className="flex justify-between items-center p-2 border-b">
                    <div>
                      <p><strong>Work Order Number:</strong> {workOrder.workOrderNumber}</p>
                      <p><strong>Department:</strong> {workOrder.dept}</p>
                      {editingId === workOrder._id ? (
                        ...
                        <>
                          <p><strong>Address:</strong> {workOrder.StateDetails.stateAddress}</p>
                          <p><strong>State Rate:</strong> {workOrder.StateDetails.statePayRate}</p>
                        </>
                      )}
                    </div>
                    <div className="flex space-x-2 px-8">
                      {editingId === workOrder._id ? (
                        <button
                          onClick={() => handleSave(editedWorkOrder, workOrder._id)}
