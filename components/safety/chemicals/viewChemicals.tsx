...
const ViewChemicals = () => {
  ...
  const handleDelete = async (id: any) => {
    const resp = await chemicalAction.DELETE.deleteChemical(id);
    console.warn(resp);
    if (resp.success) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const resp = await chemicalAction.UPDATE.updateChemical(
      selectedOption,
      JSON.stringify({
        quantity: quantity,
      })
    ...
    console.warn(resp);
    if (resp.success) {
      toast.success("Chemical Updated,Reload to view Changes");
    } else {
      toast.error("An Error Occurred");
    }
  };
