...
const FilterForm = () => {
    ...
    const toDateParsed = new Date(toDate);
      ...
      if (uniqueEmployees.size > 0) {
        ...
        uniqueEmployees.forEach((item) => {
            info.find((item2) => item2.employee?._id === item && item2.employee?.appointmentDate && isWithinRange(item2.employee.appointmentDate)) ? workerJoined += 1 : workerJoined += 0;

            info.find((item2) => item2.employee?._id === item && item2.employee?.resignDate && isWithinRange(item2.employee.resignDate)) ? workerExits += 1 : workerExits += 0;
        })
      }
  ...
  const handleSubmit = async (e) => {
    ...
    const allWages = await analyticsAction.FETCH.fetchHRAnalytics(fromDate, toDate);
        if (allWages.success) {
            ...
            if (selectedWorkOrder === "all") {
                ...
                const filteredData = parsedData.filter((item) => item.workOrderHr === selectedWorkOrder);
    ...
    <div className="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
      <form onSubmit={handleSubmit} className='p-10'>
        <div className="flex flex-row gap-4 mb-4">
          ...
          <div className="flex-1">
            <label className="block text-gray-700 text-sm font-bold mb-2" htmlFor="workOrder">
              Work Orders:
            </label>
            <select
        ...
        <div className="flex items-center justify-center">
          <button
            className={`
              ...
              ${!(fromDate && toDate && selectedWorkOrder) ? 'bg-indigo-400' : 'bg-indigo-600'} 
              focus:outline-none
    ...
    {isLoading ? (
        ...
        <div className="mt-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <div className="p-8 bg-white shadow-xl rounded-xl border border-gray-300 hover:scale-105 transition duration-300 cursor-pointer">
            <p className="text-3xl font-extrabold text-gray-800">{formatter.format(analyticsData.grossWage)}</p>
            <p className="text-lg font-light text-gray-500">Gross Wages</p>
          </div>
          <div className="p-8 bg-white shadow-xl rounded-xl border border-gray-300 hover:scale-105 transition duration-300 cursor-pointer">
            <p className="text-3xl font-extrabold text-gray-800">{formatter.format(analyticsData.netPayment)}</p>
            <p className="text-lg font-light text-gray-500">Net Payment</p>
          </div>
          <div className="p-8 bg-white shadow-xl rounded-xl border border-gray-300 hover:scale-105 transition duration-300 cursor-pointer">
            <p className="text-3xl font-extrabold text-gray-800">{formatter.format(analyticsData.pf)}</p>
            <p className="text-lg font-light text-gray-500">PF</p>
          </div>
          <div className="p-8 bg-white shadow-xl rounded-xl border border-gray-300 hover:scale-105 transition duration-300 cursor-pointer">
            <p className="text-3xl font-extrabold text-gray-800">{formatter.format(analyticsData.esic)}</p>
            <p className="text-lg font-light text-gray-500">ESIC</p>
          </div>
          <div className="p-8 bg-white shadow-xl rounded-xl border border-gray-300 hover:scale-105 transition duration-300 cursor-pointer">
            <p className="text-3xl font-extrabold text-gray-800">{analyticsData.numberOfEmployees}</p>
            <p className="text-lg font-light text-gray-500">Number of Employees</p>
          </div>
          <div className="p-8 bg-white shadow-xl rounded-xl border border-gray-300 hover:scale-105 transition duration-300 cursor-pointer">
            <p className="text-3xl font-extrabold text-gray-800">{analyticsData.workerJoined}</p>
            <p className="text-lg font-light text-gray-500">Worker Joined</p>
          </div>
          <div className="p-8 bg-white shadow-xl rounded-xl border border-gray-300 hover:scale-105 transition duration-300 cursor-pointer">
            <p className="text-3xl font-extrabold text-gray-800">{analyticsData.workerExits}</p>
            <p className="text-lg font-light text-gray-500">Worker Exits</p>
          </div>
        </div>
      ) : (
        <div className="mt-8 flex items-center justify-center">
          <h1 className="text-3xl font-extrabold text-center text-gray-300">Please select date and work order</h1>
        </div>
          )}
    </div>
    </>
